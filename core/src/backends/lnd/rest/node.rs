...
use crate::types::{
    CreateInvoiceParams, CreateInvoiceResult, NodeInfo, PayInvoiceParams, PayInvoiceResult, // CreateChannelParams, CreateChannelResult
};

use super::config::LndRestConfig;
use super::types::{
    ApiError, CreateInvoiceRequest, CreateInvoiceResponse, GetInfoResponse, SendPaymentSyncRequest,
    SendPaymentSyncResponse, // CreateChannelRequest, CreateChannelResponse
};
...
impl NodeMethods for LndRest {
    ...
    async fn create_channel(
        &self,
        invoice: CreateChannelParams,
    ) -> Result<CreateChannelResult, Error> {
        let url = format!("{}/v1/channels/stream", self.config.url);

        let request: CreateChannelRequest = invoice.into();
        let mut response = self.client.post(&url).json(&request).send().await?;

        response = Self::on_response(response).await?;

        let data: CreateChannelResponse = response.json().await?;
